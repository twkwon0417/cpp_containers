...
public:
  ...
  // Template converting constructor for pair<U1, U2> -> pair<T1, T2>
  ...
  template <typename U1, typename U2>
  pair &operator=(
      const pair<U1, U2> &other); // 자기 자신의 상태를 직접 바꾸는 연산은
                                  // 객체지향원칙에 따라 그 객체의 기능, 즉 멤버
                                  // 함수로 구현하는 것이 가장 자연스럽다.
};
...
// Non-member function
// 대칭성: a == b가 참이면 b == a도 참이어야 합니다. 비교는 대칭적 관계입니다.
// a가 주체라기보다는 a와 b를 동등하게 놓고 비교하는 것에 가깝 유연성 (가장
// 중요한 이유): 비교 연산자를 일반 함수로 만들면 암시적 타입 변환을 양쪽
// 피연산자에 모두 자유롭게 적용할 수 있다.
template <typename T1, typename T2>
